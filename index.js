...
app.get("/agendamentos", (req, res) => {
    const cpf_cliente = req.query.cpf || ""; // Recebe o CPF da query string (se houver)

    if (cpf_cliente) {
        // Se CPF foi passado, busca clientes que possuam esse CPF ou parte dele
        const query = `SELECT * FROM agendamentos WHERE cpf_cliente LIKE ?`;

        db.all(query, [`%${cpf_cliente}%`], (err, rows) => {
            if (err) {
                console.error(err);
                return res
                    .status(500)
